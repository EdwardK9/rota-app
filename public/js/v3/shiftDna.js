...
V3.register('shift-dna', '🧬 Shift DNA', {
  ...
  async init() {
    document.getElementById('view-shift-dna').innerHTML = V3.loading('Sequencing your rota…');
    await this.load();
  },

  async load() {
    try {
      this.render(await V3.api.shiftDna(this.year));
    } catch (e) {
  ...
  _wire() {
    const sel = document.getElementById('dnaYear');
    if (sel) sel.addEventListener('change', e => { this.year = e.target.value; this.load(); });
  },

  render(d) {
    const el = document.getElementById('view-shift-dna');
    const picker = V3.yearPicker('dnaYear', this.year, true);

    if (!d.shift_count) {
      el.innerHTML = picker + V3.empty('🧬', 'No completed shifts for this period',
        'Pick another year, or complete a few shifts and come back.');
      this._wire();
      return;
    }
    ...
    el.innerHTML = `
      ...
      <div class="v3-hero" style="background:linear-gradient(135deg,#EC4899,#831843)">
        <div class="v3-hero-label">YOUR SHIFT ARCHETYPE</div>
        <div class="v3-hero-value">${a.icon} ${esc(a.name)}</div>
        <div class="v3-hero-sub">
          ${esc(a.blurb)}
          ${a.based_on.length ? `<br><br>Based on your strongest traits: ${a.based_on.map(esc).join(' + ')}.` : ''}
        </div>
      </div>
      ...
      <div class="v3-grid v3-grid-lg">
        <div class="card">
          ...
          <div class="card-body">
            ...
            <div class="v3-note">
              Every trait is a plain measurement of your own shifts, not a judgement — high
              "Variety" just means your start times move around, which may well be someone
        ...
        <div class="card">
          ...
          <div class="card-body">
            ...
            <div class="v3-section-title">🕐 Hours you're usually on the clock</div>
            <div class="v3-hours">
            ...
            ${p.busiest_shift ? `<div class="v3-note">
              Your busiest day was ${fmtDate(p.busiest_shift.date)}, with ${p.busiest_shift.crew}
              colleagues overlapping your shift.
            </div>` : ''}
          </div>
        </div>
      </div>
